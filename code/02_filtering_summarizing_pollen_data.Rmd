...
check01 <- pollen02 %>%
  group_by(year, bird_id, day_bird_id, patch, control_treatment, exp_phase, date, slide_number, slide_id, band_number, colors, pollen_yes_or_no, bird_species, bird_scientific_name, bird_sex, pollen_morphotype) %>%
  summarise(n = n()) %>%
  filter(n > 1)

#Summarize pollen data so that there are not duplicate pollen morphotypes on a single slide (pollen sample)
pollen03 <- pollen02 %>%
  select(year, patch, bird_id, day_bird_id, control_treatment, exp_phase, date, slide_number, slide_id, band_number, colors, pollen_yes_or_no, bird_species, bird_scientific_name, bird_sex, contains("name"), pollen_morphotype, record_in_capture_data, collection_time, pollen_count) %>%
...
Remove samples with no pollen, after calculating the percentage of slides without pollen
```{r step3}
...
#Also add grouping variable for bird group
pollen06 <- pollen05 %>%
...
```{r step5}
data_for_normal_visitation <- pollen06 %>%
...
morphotypes_per_slide <- data_for_experiment %>%
  select(year, patch, control_treatment, exp_phase, date, slide_number, slide_id, bird_id, day_bird_id, band_number, bird_species, bird_scientific_name, bird_sex, recap_y_n, pollen_yes_or_no, pollen_morphotype) %>%
  group_by(year, patch, control_treatment, exp_phase, date, slide_number, slide_id, bird_id, day_bird_id, band_number, bird_species, bird_scientific_name, bird_sex, recap_y_n, pollen_yes_or_no) %>%
...
Decided to just look at 'normal' data, but results are basically the same if decide to include all slides
```{r step8a}
morphotypes_per_slide_normal <- data_for_normal_visitation %>%
  select(year, patch, control_treatment, exp_phase, date, slide_number, slide_id, bird_id, day_bird_id, band_number, bird_species, bird_scientific_name, bird_sex, pollen_yes_or_no, pollen_morphotype) %>%
  group_by(year, patch, control_treatment, exp_phase, date, slide_number, slide_id, bird_id, day_bird_id, band_number, bird_species, bird_scientific_name, bird_sex, pollen_yes_or_no) %>%
  summarise(num_morphotypes = n()) %>%
  ungroup() %>%
  arrange(num_morphotypes)
```
...
write.csv(data_for_experiment, "../data/export/intermediate/Pollen_data_filtered_for_experiment.csv") 
write.csv(data_for_normal_visitation, "../data/export/intermediate/Pollen_data_filtered_for_metanetwork.csv") #'normal' visitation
write.csv(data_for_sampling_method, "../data/export/intermediate/Pollen_data_filtered_for_sampling_method.csv")

#Summarized data for analysis

#Summarized morphotype richness
write.csv(morphotypes_per_slide, "../data/export/for_analysis/Pollen_data_summarized_for_pp_richness.csv") 

#Interaction frequencies between plants + hummingbirds (per replicate)
write.csv(frequencies_for_pp_networks, "../data/export/for_analysis/Pollen_data_summarized_for_pp_networks.csv")

#Interaction frequencies between plants + hummingbirds (meta-network)
write.csv(frequencies_for_metanetwork, "../data/export/for_analysis/Pollen_data_summarized_for_metanetwork.csv") 
```
