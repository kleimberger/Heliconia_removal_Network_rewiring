...
```{r step3c, message = FALSE, warning = FALSE}
...
make_specialization_plot <- function(emmeans_data, yvar){
  ...
  plot <- emmeans_data %>%
            ...
            ggplot(data = ., aes(x = sampling_method, y = estimate)) + 
              ...
              geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), position = position_dodge(width = 0.25), width = 0.0, size = 1) +
...
#Q1
write.csv(specialization_emmeans, "../results/analyses_sampling_method/tables/Specialization_results_emmeans.csv", row.names = FALSE)
write.csv(specialization_contrasts, "../results/analyses_sampling_method/tables/Specialization_results_contrasts.csv", row.names = FALSE) 
