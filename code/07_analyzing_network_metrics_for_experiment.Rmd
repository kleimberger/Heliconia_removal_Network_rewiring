...
**Check assumptions**
```{r step3b, message = FALSE, fig.show = 'hide', results = 'hide', eval = FALSE}
...
```{r step3c, message = FALSE, fig.show = 'hide', results = 'hide', eval = FALSE}
...
```{r step5b, message = FALSE, fig.width = 9, fig.height = 9}
...
interaction_plots <- emmeans_for_plots %>%
  ...
  mutate(interaction_plot = pmap(list(ggeffects, yvar, ymax, sampling_method), ~make_interaction_plot(ggeffects_df = ..1, yvar = ..2, ymax = ..3, sampling_method = ..4, add_icon = TRUE))) %>%
...
# interaction_multiplot_individual
# ggview(units = "in", width = 9, height = 9)
...
# interaction_multiplot_species_network_all
# ggview(units = "in", width = 9, height = 12)
...
# interaction_multiplot_species_network_gv
# ggview(units = "in", width = 9, height = 9)
```
...
Individual specialization
```{r step5c, message = FALSE, warning = FALSE, fig.width = 9, fig.height = 9}
#INDIVIDUAL SPECIALIZATION
contrast_plot_individual <- contrasts_of_contrasts %>%
  filter(analysis == "specialization_individual") %>%
  mutate(subanalysis = factor(subanalysis, levels = c("all_individuals", "recaptures"), labels = c("All individuals", "Recaptures only"))) %>%
  make_contrast_plot(contrasts_df = ., xvar = "subanalysis", shading = "above") +
    ...
    theme(legend.position = "bottom",
          plot.margin = unit(c(5, 18, 0.5, 0.5), "lines")) + #Default plot margins in theme_bw are 1, 1, 0.5, 0.5 (top, right, bottom, left)
    ...
    annotate("text",  x = 3.5, y = 2.25, size = 6, angle = 0, color = "black", label = c("Pre-to-post \ndecrease in specialization \nrelative to controls")) +
    annotate("text",  x = 3.5, y = 0.4, size = 6, angle = 0, color = "black", label = c("Pre-to-post \nincrease in specialization \nrelative to controls")) +
    annotation_custom(rasterGrob(pollen_icon, width = 0.25), xmin = 1, xmax = 2, ymin = 5, ymax = Inf)
...
Interaction turnover
```{r step5d, message = FALSE, warning = FALSE, fig.width = 9, fig.height = 9}
#INTERACTION TURNOVER
contrast_plot_turnover <- contrasts_of_contrasts %>%
  filter(analysis == "turnover" & subanalysis == "binary") %>%
  mutate(yvar = factor(yvar, levels = c("WN", "ST", "OS"), labels = c("Total turnover", "Species turnover", "Among-species turnover"))) %>%
  make_contrast_plot(contrasts_df = ., xvar = "yvar", shading = "above") +
    ...
    annotate("text",  x = 4.85, y = 1.85, size = 6, angle = 0, color = "black", label = c("Pre-to-post \nincrease in turnover \nrelative to controls")) +
    annotate("text",  x = 4.85, y = 0.4, size = 6, angle = 0, color = "black", label = c("Pre-to-post \ndecrease in turnover \nrelative to controls")) +
    annotation_custom(rasterGrob(camera_icon, width = 0.25), xmin = 1, xmax = 3, ymin = 3.75, ymax = Inf)

contrast_plot_turnover
ggview(units = "in", width = 9, height = 9)
```

Species and network-level specialization
```{r step5e, message = FALSE, warning = FALSE, fig.width = 14, fig.height = 12}
...
contrast_plot_pollen_species <- contrasts_of_contrasts %>%
  ...
  make_contrast_plot(contrasts_df = ., xvar = "yvar", shading = "below") +
    ...
    theme(legend.position = "bottom", plot.margin = unit(c(5, 1, 0.5, 0.5), "lines")) + #Default plot margins in theme_bw are 1, 1, 0.5, 0.5 (top, right, bottom, left)
    annotation_custom(rasterGrob(pollen_icon, width = 0.25*1.5), xmin = 1, xmax = 2, ymin = 6, ymax = Inf)

# contrast_plot_pollen_species
# ggview(units = "in", width = 9, height = 6)
...
contrast_plot_pollen_network <- contrasts_of_contrasts %>%
  ...
  make_contrast_plot(contrasts_df = ., xvar = "yvar", shading = "below") +
    ...
    annotation_custom(rasterGrob(pollen_icon, width = 0.25), xmin = 0.5, xmax = 1.5, ymin = 6, ymax = Inf)
...
contrast_plot_camera_species <- contrasts_of_contrasts %>%
  ...
  make_contrast_plot(contrasts_df = ., xvar = "yvar", shading = "below") +
    ...
    labs(title = "Species specialization") +
    theme(legend.position = "bottom",
          plot.margin = unit(c(5, 1, 0.5, 0.5), "lines")) + #Default plot margins in theme_bw are 1, 1, 0.5, 0.5 (top, right, bottom, left)
    annotation_custom(rasterGrob(camera_icon, width = 0.25*1.5), xmin = 1, xmax = 2, ymin = 3, ymax = Inf)
...
contrast_plot_camera_network <- contrasts_of_contrasts %>%
  ...
  make_contrast_plot(contrasts_df = ., xvar = "yvar", shading = "below") +
    ...
    annotate("text",  x = 2.18, y = 0.4, size = 6, angle = 0, color = "black", label = c("Pre-to-post \ndecrease in specialization \nrelative to controls")) +
    annotation_custom(rasterGrob(camera_icon, width = 0.25), xmin = 0.5, xmax = 1.5, ymin = 3, ymax = Inf)
...
contrast_multiplot_species_network <- ggarrange(contrast_plot_pollen_species, contrast_plot_pollen_network, contrast_plot_camera_species, contrast_plot_camera_network,
                                                common.legend = TRUE, ncol = 2, nrow = 2, align = "h", legend = "bottom", labels = "AUTO", font.label = list(size = 24, face = "plain"), widths = c(1, 1.5))
contrast_multiplot_species_network
ggview(units = "in", width = 14, height = 12)
```
...
write.csv(results_tidy_confint, file.path(path_name, "Network_rewiring_results_summary_tables.csv"), row.names = FALSE)
write.csv(sample_size, file.path(path_name, "Network_rewiring_results_sample_size_summary.csv"), row.names = FALSE)
write.csv(emmeans, file.path(path_name,"Network_rewiring_results_emmeans.csv"), row.names = FALSE)
write.csv(contrasts, file.path(path_name,"Network_rewiring_results_contrasts.csv"), row.names = FALSE)
write.csv(contrasts_of_contrasts, file.path(path_name, "Network_rewiring_results_contrasts_of_contrasts.csv"), row.names = FALSE)

#FIGURES
path_name <- c("../results/analyses_experiment/figures")

#Interaction plots
ggsave(file.path(path_name, "Interaction_multiplot_species_and_network_all.png"), interaction_multiplot_species_network_all, dpi = 300, units = "in", width = 9, height = 12, bg = "white")
ggsave(file.path(path_name, "Interaction_multiplot_species_and_network_gv.png"), interaction_multiplot_species_network_gv, dpi = 300, units = "in", width = 9, height = 9, bg = "white")
ggsave(file.path(path_name, "Interaction_multiplot_individual.png"), interaction_multiplot_individual, dpi = 300, units = "in", width = 9, height = 9, bg = "white")

#Contrast plots
ggsave(file.path(path_name, "Contrast_individual.png"), contrast_plot_individual, dpi = 300, units = "in", width = 11, height = 9, bg = "white")
ggsave(file.path(path_name, "Contrast_turnover.png"), contrast_plot_turnover, dpi = 300, units = "in", width = 9, height = 9, bg = "white")
ggsave(file.path(path_name, "Contrast_multiplot_species_and_network.png"), contrast_multiplot_species_network, dpi = 300, units = "in", width = 14, height = 12, bg = "white")
```