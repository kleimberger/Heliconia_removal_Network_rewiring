...
yvars = c("WN", "ST", "OS")
yvars_names = data.frame(yvars = yvars, yvars_names = c("Interaction turnover", "Species turnover", "Interaction turnover (shared species)"))
...
base_table <- crossing(yvars, types) %>% 
  ...
  mutate(model_id = paste(model_number, "camera", types, sep = "_")) %>%
  mutate(title = paste("camera", types, sep = "_")) %>%
...
path_name <- c("../results/analyses_experiment/interaction_turnover/assumption_checks")
...
influence_checks <- models %>%
  ...
  mutate(plotname4 = paste("influence_diagnostics_", model_id, ".png", sep = ""))
...
**Plot results**
```{r step3f, message = FALSE, fig.width=12, fig.height=4}
...
plots_binary <- plots %>%
  mutate(yvars = factor(as.character(yvars), levels = c("WN", "ST", "OS"), ordered = TRUE)) %>%
  filter(types == "binary") %>%
  arrange(yvars)
...
#Summary tables
write.csv(results_tidy_confint, file.path(path_name, "tables/Turnover_results_summary_tables.csv"))
write.csv(sample_size, file.path(path_name, "tables/Turnover_results_sample_size_summary.csv"))
write.csv(emmeans, file.path(path_name, "tables/Turnover_results_emmeans.csv"))
write.csv(contrasts, file.path(path_name, "tables/Turnover_results_contrasts.csv"))

#Plots
ggsave(file.path(path_name, "figures/Turnover_multiplot_binary.png"), multiplot_binary, dpi = 300, units = "in", width = 12, height = 4, bg = "white")
ggsave(file.path(path_name, "figures/Turnover_multiplot_quant.png"), multiplot_quant, dpi = 300, units = "in", width = 12, height = 4, bg = "white")
```
