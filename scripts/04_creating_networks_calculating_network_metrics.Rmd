...
#Functions to build networks and calculate metrics
source("../scripts/helper_functions/Build_and_visualize_networks_for_experiment.R")
...
#Functions to extract the results from safely/quitely
...
#2. Create matrix
#3. Remove networks any without matrices
...
network_results <- base_table %>%
  ...
  filter(matrix_error == "NULL") %>%
  mutate(network_plot = map2(matrix, sampling_method, ~visualize_network(matrix = .x, method_name = .y, label_heto = TRUE))) %>%
  mutate(rowname = 1:length(year)) %>%
  mutate(plotname = paste(paste(rowname, year, patch, exp_phase, sep = "_"), ".png", sep = "")) %>%
...
specieslevel_errors <- specialization_results %>%
  filter(specieslevel_error != "NULL")

specieslevel_warnings <- specialization_results %>%
  filter(specieslevel_warning != "NULL")
```
...
#To compute binary AND quantitative indices
binary = c(TRUE, FALSE)
...
#3. Prepare to compute binary AND quantitaive indices
...
turnover_results <- base_table_turnover %>%
  mutate(array = map2(pre, post, ~bipartite::webs2array(list(.x, .y)))) %>%
  mutate(betalink_fruend = map2(array, binary, ~bipartite::betalinkr(.x, partitioning = "commondenom", index = "bray", partition.st = FALSE, partition.rr = FALSE, binary = .y))) %>% 
  mutate(betalink_poisot = map2(array, binary,  ~bipartite::betalinkr(.x, partitioning = "poisot", index = "bray", partition.st = FALSE, partition.rr = FALSE, binary = .y, distofempty = "na"))) 

#Testing out the different settings for partitioning...
(test01 = bipartite::betalinkr(bipartite::webs2array(base_table_turnover$pre[[1]], base_table_turnover$post[[1]]), partitioning = "commondenom", index = "bray", binary = TRUE))
(test02 = bipartite::betalinkr(bipartite::webs2array(base_table_turnover$pre[[1]], base_table_turnover$post[[1]]), partitioning = "poisot", index = "bray", binary = TRUE))
```
...
```{r step6}
write.csv(specialization_results_for_analysis, "../data/export/for_analysis/Network_specialization_for_analysis.csv")
write.csv(turnover_results_for_analysis, "../data/export/for_analysis/Network_dissimilarity_for_analysis.csv")
write.csv(completeness_results_for_analysis, "../data/export/for_analysis/Sampling_completeness_for_analysis.csv")

setwd("../results/figures/pre_post_networks_experiment")
walk2(network_results$plotname, network_results$network_plot, ~ggplot2::ggsave(filename = .x, plot = .y, height = 12, width = 10, bg = "white", device = "png"))
```
