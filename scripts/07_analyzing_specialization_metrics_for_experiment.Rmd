...
analysis_network <- c("pollen", "camera")
...
subset_data <- function(dataset, analysis, bird_group, yvar){
  ...
  if(yvar != "num_morphotypes"){
    ...
    if(analysis == "camera"){dataset <- dataset %>% filter(sampling_method == "visitation")}
...
create_model <- function(dataset, yvar, analysis = NA){
  
  #Individual specialization (# pollen morphotypes/bird)
  if(yvar == "num_morphotypes"){
    
    if(analysis == "pollen_all_birds"){
    ...
    if(analysis == "pollen_recaps"){
      
      model <- eval(bquote(glmmTMB(.(as.formula(paste(yvar, "~ control_treatment * exp_phase + (1|patch/year_patch/bird_id)"))), data = dataset, na.action = na.omit, family = "truncated_genpois", control = glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS")))))
...
test_data02 <- subset_data(dataset = network_data, analysis = "camera", bird_group = "all_spp", yvar = "H2")
...
assumption_checks_network <- models_network %>%
  ...
  mutate(plotname1 = paste("re_diagnostics_network_", model_number, ".png", sep = "")) %>%
  mutate(plotname2 = paste("dharma_overall_diagnostics_network_", model_number, ".png", sep = "")) %>%
  mutate(plotname3 = paste("dharma_xvar_diagnostics_network_", model_number, ".png", sep = ""))
...
# models_network$model[[10]] %>% DHARMa::simulateResiduals() %>% plot()

#Export diagnostic plots because they load very slowly in RStudio
path_name <- c("../results/analyses_experiment/specialization/assumption_checks")
walk2(assumption_checks_network$plotname1, assumption_checks_network$re_plot, ~ggsave(filename = .x, plot = .y, path = path_name, height = 11.5, width = 15, units = "in", bg = "white"))
walk2(assumption_checks_network$plotname3, assumption_checks_network$dharma_xvar_plot, ~ggsave(filename = .x, plot = .y, path = path_name, height = 11.5, width = 15, units = "in"))

#Remove problematic plot before trying to export DHARMa plots
assumption_checks_network <- assumption_checks_network %>% filter(model_number != "10")
walk2(assumption_checks_network$plotname2, assumption_checks_network$dharma_plot, ~ggsave(filename = .x, plot = .y, path = path_name, height = 11.5, width = 15, units = "in"))
```
...
influence_checks_network <- models_network %>%
...
#Export plots
path_name <- c("../results/analyses_experiment/specialization/assumption_checks")
walk2(influence_checks_network$plotname4, influence_checks_network$influence_plot, ~ggsave(filename = .x, plot = .y, path = path_name, height = 11.5, width = 15, units = "in", bg = "white"))
```
...
Interaction plots
```{r step3f, message = FALSE, fig.width=9, fig.height=12}
...
```{r step 3g, fig.width=9, fig.height=9}
...
#Contrast plot (CAMERA)
contrast_plot_camera <- contrasts_x2_network %>%
  filter(analysis == "camera") %>%
  make_contrast_plot(estimate_name = "odds.ratio", xaxis_name = "yvars", shading = "below") +
  scale_x_discrete("Specialization", labels = c(h2_label, d_label, ssi_label)) +
  scale_y_continuous(limits = c(0, 6), breaks = c(0, 0.5, 1, 2, 3, 4, 5, 6)) +
  coord_cartesian(ylim = c(0, 6), xlim = c(1, 3), clip = "off") +
...
#Contrast plot (POLLEN + CAMERA)
contrast_multiplot_network <- ggarrange(contrast_plot_pollen, contrast_plot_camera, ncol = 2, align = "h", common.legend = TRUE, legend = c("top"))
```
...
models_richness <- data_richness %>%
  mutate(model_quietly = pmap(list(data, analysis), quietly(function(a, b){model <- create_model(dataset = a, analysis = b, yvar = "num_morphotypes")})),
         model = map(model_quietly, get_result),
         warning = map(model_quietly, get_warning), 
         convergence = map(model, check_convergence)) %>%
  mutate(xvar_table = map(model, ~data.frame(xvar = all.vars(terms(.))[-1])))
...
assumption_checks_richness <- models_richness %>%
  ...
  mutate(dharma_xvar_plot = pmap(list(xvar_table, data, dharma_object, title, subtitle),
                                 ~make_dharma_xvar_plot(predictor_table = ..1, dataset = ..2, dharma_object = ..3, plot_title = ..4, plot_subtitle = ..5))) %>%
  mutate(plotname1 = paste("re_diagnostics_individual_", model_number, ".png", sep = "")) %>%
  mutate(plotname2 = paste("dharma_overall_diagnostics_individual_", model_number, ".png", sep = "")) %>%
  mutate(plotname3 = paste("dharma_xvar_diagnostics_individual_", model_number, ".png", sep = ""))
        
#Export diagnostic plots because they load very slowly in RStudio
path_name <- c("../results/analyses_experiment/specialization/assumption_checks")
walk2(assumption_checks_richness$plotname1, assumption_checks_richness$re_plot, ~ggsave(filename = .x, plot = .y, path = path_name, height = 11.5, width = 15, units = "in", bg = "white"))
walk2(assumption_checks_richness$plotname2, assumption_checks_richness$dharma_plot, ~ggsave(filename = .x, plot = .y, path = path_name, height = 11.5, width = 15, units = "in"))
walk2(assumption_checks_richness$plotname3, assumption_checks_richness$dharma_xvar_plot, ~ggsave(filename = .x, plot = .y, path = path_name, height = 11.5, width = 15, units = "in"))
```

**Check for highly influential replicates**
```{r step4d, message = FALSE, fig.show = 'hide', results = 'hide'}
#Make plots of DFBETAS and Cook's distance
influence_checks_richness <- models_richness %>%
  mutate(influence_plot = pmap(list(model, data, title, subtitle), ~make_influence_plot(model = ..1, dataset = ..2, plot_title = ..3, plot_subtitle = ..4, group_id = "year_patch"))) %>%
  mutate(plotname4 = paste("influence_diagnostics_individual_", model_number, ".png", sep = ""))

#Export plots
path_name <- c("../results/analyses_experiment/specialization/assumption_checks")
walk2(influence_checks_richness$plotname4, influence_checks_richness$influence_plot, ~ggsave(filename = .x, plot = .y, path = path_name, height = 11.5, width = 15, units = "in", bg = "white"))
```
...
Interaction plots
```{r step4g, message = FALSE, fig.width=9, fig.height=9}
...
Contrast plot
```{r step4h, message = FALSE, fig.width=6, fig.height=6}
...
#Contrast plot (CAMERA, NETWORK-LEVEL)
contrast_plot_camera_network <- contrasts_x2_network %>%
  filter(analysis == "camera" & yvars == "H2") %>%
  make_contrast_plot(estimate_name = "odds.ratio", xaxis_name = "yvars", shading = "below") +
  scale_x_discrete(labels = c(h2_label)) +
  scale_y_continuous(limits = c(0, 4), breaks = c(0, 0.5, 1, 2, 3, 4)) +
  coord_cartesian(ylim = c(0, 4), xlim = c(1, 1), clip = "off") +
...
#Contrast plot (CAMERA, SPECIES-LEVEL)
contrast_plot_camera_species <- contrasts_x2_network %>%
  filter(analysis == "camera" & (yvars == "d" | yvars == "species.specificity.index")) %>%
  make_contrast_plot(estimate_name = "odds.ratio", xaxis_name = "yvars", shading = "below") +
  scale_x_discrete(labels = c(d_label, ssi_label)) +
  scale_y_continuous(limits = c(0, 6), breaks = c(0, 0.5, 1, 2, 3, 4, 5, 6)) +
  coord_cartesian(ylim = c(0, 6), xlim = c(1, 2), clip = "off")
...
#Combine...
contrast_multiplot_all_analyses <- ggarrange(contrast_plot_pollen_network, contrast_plot_camera_network, contrast_plot_pollen_species, contrast_plot_camera_species, contrast_plot_richness, common.legend = TRUE, ncol = 2, nrow = 3, legend = "bottom")
...
#SUMMARY TABLES
path_name <- c("../results/analyses_experiment/specialization/tables")

#Network
write.csv(results_tidy_confint_network, file.path(path_name, "Network_results_summary_tables.csv"))
write.csv(sample_size_network, file.path(path_name, "Network_results_sample_size_summary.csv"))
write.csv(emmeans_network, file.path(path_name,"Network_results_emmeans.csv"))
write.csv(contrasts_network, file.path(path_name, "Network_results_contrasts.csv"))
write.csv(contrasts_x2_network, file.path(path_name,"Network_results_contrast_of_contrasts.csv"))

#Individual
write.csv(results_tidy_confint_richness, file.path(path_name, "Individual_results_summary_tables.csv"))
write.csv(sample_size_richness, file.path(path_name, "Individual_results_sample_size_summary.csv"))
write.csv(emmeans_richness, file.path(path_name, "Individual_results_emmeans.csv"))
write.csv(contrasts_richness, file.path(path_name, "Individual_results_contrasts.csv"))
write.csv(contrasts_x2_richness, file.path(path_name, "Individual_results_contrast_of_contrasts.csv"))

#FIGURES
path_name <- c("../results/analyses_experiment/specialization/figures")

#Network
ggsave(file.path(path_name, "Network_interaction_multiplot_all_species.png"), interaction_multiplot_network_all, dpi = 300, units = "in", width = 9, height = 12, bg = "white")
ggsave(file.path(path_name, "Network_interaction_multiplot_greh_visa.png"), interaction_multiplot_network_gv, dpi = 300, units = "in", width = 9, height = 9, bg = "white")
ggsave(file.path(path_name, "Network_contrast_multiplot.png"), contrast_multiplot_network, dpi = 300, units = "in", width = 13, height = 5, bg = "white")

#Individual
ggsave(file.path(path_name, "Individual_interaction_multiplot.png"), interaction_multiplot_richness, dpi = 300, units = "in", width = 9, height = 9, bg = "white")

#All analyses
ggsave(file.path(path_name, "Network_and_individual_contrast_multiplot.png"), contrast_multiplot_all_analyses, dpi = 300, units = "in", width = 9, height = 12, bg = "white")
```