...
h2_label <- expression(italic(H[2])*"'")
d_label <- expression(italic(d)*"' (mean)")
ssi_label <- expression("SSI~(mean)")

#As density plot
specialization_density_plot <- data_long %>%
  mutate(sampling_method = factor(sampling_method, levels = c("pollen", "visitation"), labels = c("Pollen", "Visitation"))) %>%
...
specialization_predictions_contrasts <- specialization_models %>%
  mutate(ggeffects = map(model, ~ggpredict(., terms = c("sampling_method"), back.transform = TRUE))) %>%
  mutate(emm_contrasts = map(model, ~calculate_contrasts(.)))

#Extract estimated marginal means
specialization_predictions <- specialization_predictions_contrasts %>%
  select(model_number, metric, ggeffects) %>% 
  unnest(cols = c(ggeffects))

#Extract contrasts
specialization_contrasts <- specialization_predictions_contrasts %>%
...
make_specialization_plot <- function(ggeffects, yvar){
  ...
  plot <- ggeffects %>%
            mutate(x = factor(x, levels = c("pollen", "visitation"), labels = c("Pollen", "Visitation"))) %>%
  ...
  if(yvar == "H2"){plot <- plot + labs(y = expression(atop("Network specialization", paste("("*italic(H[2])*"')"))))}
  if(yvar == "d"){plot <- plot + labs(y = expression(atop("Species specialization", paste("(mean "*italic(d)*"')"))))}
  if(yvar == "species.specificity.index"){plot <- plot + labs(y = expression(atop("Species specialization", paste("(mean SSI)"))))}

  return(plot)
  
}

specialization_results_plots <- specialization_predictions_contrasts %>%
  mutate(predicted_plot = map2(ggeffects, metric, ~make_specialization_plot(ggeffects = .x, yvar = .y)))

#Combine into one plot...
specialization_results_multiplot <- ggarrange(plotlist = specialization_results_plots$predicted_plot, nrow = 3)
specialization_results_multiplot
```
...
#Combine into one plot...
correlation_results_multiplot <- ggarrange(plotlist = correlation_results_plots$predicted_plot, nrow = 3)
correlation_results_multiplot
```

## Step 5: Export results
```{r step5}
#Summary tables (Q1 + Q2)
write.csv(results_tidy_confint, "../results/sampling_method/tables/Summary_tables.csv") #model summaries

#Q1
write.csv(specialization_predictions, "../results/sampling_method/tables/Specialization_results_emmeans.csv")
write.csv(specialization_contrasts, "../results/sampling_method/tables/Specialization_results_contrasts.csv") 

ggsave("../results/sampling_method/figures/Specialization_density_plot.png", specialization_density_plot, dpi = 300, units = "in", width = 9, height = 5)
ggsave("../results/sampling_method/figures/Specialization_results_multiplot.png", specialization_results_multiplot, dpi = 300, units = "in", width = 4, height = 11)

#Q2
ggsave("../results/sampling_method/figures/Correlation_results_multipplot.png", correlation_results_multiplot, dpi = 300, units = "in", width = 4, height = 11)
```